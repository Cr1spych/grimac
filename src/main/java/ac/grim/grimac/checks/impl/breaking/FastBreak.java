    ...
    public void onBlockBreak(BlockBreak blockBreak) {
        if (blockBreak.action == DiggingAction.START_DIGGING) {
            ...
            if (blockDelayBalance > 1000) { // If more than a second of advantage
                if (fail("delay=" + breakDelay + "ms, type=" + blockBreak.block.getType()) && shouldModifyPackets()) {
                    blockBreak.cancel();
                }
            }
        ...
        if (blockBreak.action == DiggingAction.FINISHED_DIGGING && targetBlock != null) {
            ...
            if (blockBreakBalance > 1000) { // If more than a second of advantage
                if (fail("diff=" + diff + "ms, balance=" + blockBreakBalance + "ms, type=" + blockBreak.block.getType()) && shouldModifyPackets()) {
                    blockBreak.cancel();
                }
            }
